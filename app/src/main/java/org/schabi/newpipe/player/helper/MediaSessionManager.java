...
public class MediaSessionManager {
    ...
    public MediaSessionManager(@NonNull final Context context,
        ...
        final MediaSessionConnector.CustomActionProvider changePlayMode =
                new MediaSessionConnector.CustomActionProvider() {
                    @Override
                    public void onCustomAction(@NonNull final Player currentPlayer,
                        ...
                        switch (((PlayerMediaSession) callback).mode) {
                            case 1:
                                return new PlaybackStateCompat.CustomAction.Builder(
                                        ACTION_CHANGE_PLAY_MODE, "Repeat all",
                                        R.drawable.exo_icon_shuffle_on).build();
                            case 2:
                                return new PlaybackStateCompat.CustomAction.Builder(
                                        ACTION_CHANGE_PLAY_MODE, "Repeat none",
                                        R.drawable.exo_icon_repeat_one).build();
                            case 3:
                                return new PlaybackStateCompat.CustomAction.Builder(
                                        ACTION_CHANGE_PLAY_MODE, "Repeat one",
                                        R.drawable.exo_icon_repeat_all).build();
                            case 0:
                            default:
                                return new PlaybackStateCompat.CustomAction.Builder(
                                        ACTION_CHANGE_PLAY_MODE, "Shuffle",
                                        R.drawable.shuffle_disabled).build();
                        }
                    }
                };
        final MediaSessionConnector.CustomActionProvider close =
                new MediaSessionConnector.CustomActionProvider() {
                    @Override
                    public void onCustomAction(@NonNull final Player currentPlayer,
                                               @NonNull final String action,
        ...
        sessionConnector.setCustomActionProviders(changePlayMode, close);
        sessionConnector.setMetadataDeduplicationEnabled(true);
        sessionConnector.setMediaMetadataProvider(currentPlayer -> buildMediaMetadata());
    ...
    public void setCustomErrorMessage(@Nullable final CharSequence message, final int code) {
        sessionConnector.setCustomErrorMessage(message, code);
    }
