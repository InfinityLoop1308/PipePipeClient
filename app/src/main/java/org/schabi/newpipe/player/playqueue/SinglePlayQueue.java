...
public final class SinglePlayQueue extends PlayQueue {
    public SinglePlayQueue(final StreamInfoItem item) {
        super(0, Collections.singletonList(PlayerMediaItem.forStreamInfoItem(item)));
    }

    public SinglePlayQueue(final StreamInfo info) {
        super(0, Collections.singletonList(PlayerMediaItem.forQueueItem(info)));
    ...
    private static List<PlayerMediaItem> playQueueItemsOf(final List<StreamInfoItem> items) {
        ...
        for (final StreamInfoItem item : items) {
            playQueueItems.add(PlayerMediaItem.forStreamInfoItem(item));
        }
